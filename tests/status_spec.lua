...
-- Tests for Neogit status buffer coloring and push branch selection

-- Ensure lua/ is in the package path for require('neogit.status')
package.path = package.path .. ";lua/?.lua;lua/?/init.lua"

-- Mock plenary.job if running outside Neovim
package.loaded['plenary.job'] = package.loaded['plenary.job'] or {
    new = function() return setmetatable({start = function() end}, {__index = function() return function() end end}) end
}

-- Minimal vim mock for testing outside Neovim
_G.vim = _G.vim or {}
vim.api = vim.api or {}
vim.api.nvim_buf_is_valid = vim.api.nvim_buf_is_valid or function() return true end
vim.api.nvim_win_is_valid = vim.api.nvim_win_is_valid or function() return true end
...
vim.fn.getcwd = vim.fn.getcwd or function() return "/mock/project" end
vim.notify = vim.notify or function() end
vim.deepcopy = vim.deepcopy or function(orig)
